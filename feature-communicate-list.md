...
- `numberOfRecordsToShowInSearch` parameter has been used to control how many roles, services, people to display as search list by default when search is triggered. Matrix had default of 3 searches suggestions to be listed which has been left to default by applying this config mentioned above which provides `25` records to show at a time which is default search limit at the moment.
